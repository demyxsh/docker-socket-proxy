...
DEMYX_HAPROXY_VERSION="$(docker exec "$DEMYX_REPOSITORY" haproxy -v | awk '{print $3}' | sed -e 's/\r//g')"

# Replace versions
sed -i "s|alpine-.*.-informational|alpine-${DEMYX_ALPINE_VERSION}-informational|g" README.md
sed -i "s|haproxy-.*.-informational|haproxy-${DEMYX_HAPROXY_VERSION}-informational|g" README.md

# Echo versions to file
echo "DEMYX_ALPINE_VERSION=$DEMYX_ALPINE_VERSION
DEMYX_HAPROXY_VERSION=$DEMYX_HAPROXY_VERSION" > VERSION
...
git add VERSION
git commit -m "ALPINE $DEMYX_ALPINE_VERSION, HAPROXY $DEMYX_HAPROXY_VERSION"
