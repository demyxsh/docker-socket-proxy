...
# Replace versions
/bin/sed -i "s|alpine-.*.-informational|alpine-${DEMYX_DOCKER_SOCKET_PROXY_ALPINE_VERSION}-informational|g" README.md
/bin/sed -i "s|haproxy-.*.-informational|haproxy-${DEMYX_DOCKER_SOCKET_PROXY_HAPROXY_VERSION}-informational|g" README.md

# Echo versions to file
/bin/echo "DEMYX_DOCKER_SOCKET_PROXY_ALPINE_VERSION=$DEMYX_DOCKER_SOCKET_PROXY_ALPINE_VERSION
